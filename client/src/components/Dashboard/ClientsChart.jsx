...
const ClientsChart = ({ data = [] }) => {
  const chartData = useMemo(() => {
    ...
    const values = safeData.map((d) => Number(d?.count ?? 0) || 0);

    if (labels.length === 0) {
      labels.push("No clients yet");
      values.push(0);
    }
  ...
  return (
    <div className="sa-revenue-chart-container">
      <div className="sa-revenue-chart-wrapper">
        <Line data={chartData} options={options} datasetIdKey="clients" />
      </div>
    </div>
  );
};
